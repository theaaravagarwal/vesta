...
- Event merging gained a gap tolerance (`BEHAVIOR_MERGE_GAP_S`) because overlapping windows report one action several times. It is recommended but not yet the default: the evidence comes from the same three development clips that exposed the problem.
- Benchmark records under `docs/context/benchmarks/` are generated by `evaluation/record_run.py` from run artifacts, never written by hand, and every record is linked from the runs index in the public benchmark. Tests enforce the links.
